...
1. `<projectname>_icon.png` / `.jpg` / `.jpeg` (e.g., `MyProject_icon.png` for `MyProject.toe`) — static icon, generated on-demand via the Utility TOX by pulsing `Save Static Icon`. Use this for a hand-picked thumbnail that won't be overwritten on save.
2. `<projectname>_icon_temp.png` / `.jpg` / `.jpeg` — dynamic icon, auto-generated by the Utility TOX on every save. Gives you a live snapshot of your project.
3. `icon.png` / `icon.jpg` / `icon.jpeg` — generic project icon shared by all `.toe` files in the same folder.
